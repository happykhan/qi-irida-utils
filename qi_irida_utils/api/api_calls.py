...
class ApiCalls(object):
    ...
    def get_sequence_files_breakdown(self, project_id, sample_name):
        """
        API call to api/projects/project_id/sample_id/sequenceFiles
        We fetch the sample file through the project id on this route
        ...
        returns two lists of sequencefile dictionary (paired/unpaired) for given sample_id
        """

        logging.info("Getting sequence files pairs from sample '{}' on project '{}'".format(sample_name, project_id))
        ...
        try:
            url = self._get_link(sample_url, "sample/sequenceFiles/pairs",
                                 target_dict={
                                     "key": "sampleName",
                                     "value": sample_name
                                 })
            response = self._session.get(url)
            paired = response.json()['resource']['resources']
            url = self._get_link(sample_url, "sample/sequenceFiles/unpaired",
                                 target_dict={
                                     "key": "sampleName",
                                     "value": sample_name
                                 })
            response = self._session.get(url)
            unpaired = response.json()['resource']['resources']
    ...
    def download_seq_file(self, link, output, checksum=None):
        self._session.headers['Accept'] = 'application/fastq'
        data_stream = self._session.get(link)
        with open(output, 'wb') as f:
            f.write(data_stream.content)
        self._session.headers.pop('Accept')
        if checksum:
            if checksum != hashlib.sha256(open(output).read().encode('utf-8')).hexdigest():
                logging.error('Checksum mismatch on download %s ' %output)
...
def _initialize_api(client_id, client_secret, base_url, username, password, max_wait_time=20):
    ...
    _api_instance = ApiCalls(client_id, client_secret, base_url, username, password, max_wait_time)
    return _api_instance


def initialize_api_from_config(config_path):
    """
